...
double Angles::stod(const std::string& a_string) {
...
int Angles::stoi(const std::string& a_string) {
  int an_int;
  std::stringstream(a_string) >> an_int;
  return an_int;
}

double Angles::degrees2seconds(const double& a_deg, const double& a_min, const double& a_sec) {
  ...
  if (a_deg > 0) {
    ...
    if (a_min < 0)
      _min *= -1;
      // TODO big python explosion: throw Angles::Error("bad sign");
      // TODO catchable exception throw RangeError("bad sign");
